...
class CssBuilder {
  ...
  stringify() {
    ...
    const classs = this.className.join('');
    const atts = this.attrStr.join('');
    const pseudoClass = this.pseudoClassStr.join('');
    const pseudoElem = this.pseudoElementStr;
    const resultString = `${name}${id}${classs}${atts}${pseudoClass}${pseudoElem}`;
    return (resultString !== '' ? resultString : this.name);
  }
}
